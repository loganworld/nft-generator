...
const generate_images = () => {
    ...
    const generate = async ({ dirs, combineDatas, resImgPath, index }) => {
        if (dirs[index] == null) {
            await combineImages({ imageDatas: combineDatas, resImgPath: `${resImgPath}/res_${totalCount}.png` });
            totalCount++;
            return;
        }
        ...
        for (var i = 1; i <= length; i++) {
            await generate({ dirs, combineDatas: combineDatas.concat([`${dirs[index]}/c (${i}).png`]), index: index + 1 })
        }
    }
    generate(initDatas);
}
